//! Input resampling to the internal 16 kHz processing rate.
//!
//! The P.56 algorithm requires at least 16 kHz. Inputs below 16 kHz are
//! resampled with rubato's `FftFixedIn<f32>` (FFT-domain, high quality).
//! [`Resampler16k`] is **stateful**: the rubato instance carries its
//! internal overlap across `process` calls, and an input-side buffer
//! accumulates samples until a full chunk is available. Leftover input
//! samples are padded with zeros and flushed by [`Resampler16k::flush`].
//!
//! Inputs at exactly 16 kHz bypass the resampler entirely.
...
/// Stateful resampler: any input rate -> 16 kHz mono, f32.
...
impl Resampler16k {
    /// Creates a resampler for the given input rate. Rates >= 16 kHz are
    /// rejected here — only upsampled (or pass-through, handled by the
    /// caller) rates belong in this path. Actually exactly-16k is also
    /// rejected: the caller skips resampling for that case.
    ///
    /// # Errors
    ///
    ...
    pub fn flush(&mut self, total_input: u64) -> Result<Vec<f32>> {
        ...
        // matches ceil(total_input * 16000 / rate).
        let expected = ((total_input as f64) * (TARGET_RATE as f64 / self.sample_rate as f64))
            .round() as usize;
    ...
    /// Input sampling rate this resampler was configured with
    /// (diagnostic access).
    #[allow(dead_code)]
