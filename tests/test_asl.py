...
def test_integer_normalization(dtype: type, bit_depth: int) -> None:
    """Integer input divided by max value must match the float result."""
    fs = 16_000
    t = np.arange(fs, dtype=np.float64)
    sine = 0.25 * np.sin(2 * np.pi * 997 * t / fs)

    float_result = measure(sine.astype(np.float32), fs, bit_depth)

    max_val = float(2 ** (bit_depth - 1) - 1)
    ints = np.rint(sine * max_val).astype(dtype)
    int_result = measure(ints, fs, bit_depth)
...
def test_rejects_unsupported_dtype() -> None:
    ...
    with pytest.raises(ValueError, match="dtype"):
        meter.process_block(np.zeros(16, dtype=np.bool_))


def test_int32_24bit_divisor() -> None:
    """int32 with bit_depth=24 must divide by 8388607, not 2147483647."""
    fs = 16_000
    t = np.arange(fs, dtype=np.float64)
    sine = 0.5 * np.sin(2 * np.pi * 1000 * t / fs)
    int24 = np.rint(sine * 8_388_607).astype(np.int32)

    r = measure(int24, fs, bit_depth=24)
...
def test_resampling_matches_16k_reference(fs: int) -> None:
    ...
    r_low = measure(x, fs, bit_depth=16)
...
def _load_wav(path: Path) -> tuple[int, np.ndarray]:
    """Read a reference wav; 24-bit data is widened to int32.

    scipy widens 24-bit samples left-justified into int32 (low 8 bits
    zero), so shift right by 8 to restore the true 24-bit value for
    ``bit_depth=24`` normalization.
    """
...
def test_conformance_sv56demo(case: ConformanceCase, data_dir: Path) -> None:
    ...
    rate, data = _load_wav(data_dir / case.wav)
    assert rate == ref.sample_rate

    block_size = 256
    if case.bit_depth == 24:
        data = np.right_shift(data, 8)
    meter = ActiveSpeechLevelMeter(sample_rate=rate, bit_depth=ref.bit_depth, block_size=block_size)
